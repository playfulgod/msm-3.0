...
static unsigned int bravo_sdslot_status_rev0(struct device *dev)
{
	return !gpio_get_value(BRAVO_GPIO_SDMC_CD_REV0_N);
}

#define BRAVO_MMC_VDD	(MMC_VDD_165_195 | MMC_VDD_20_21 | \
...
static int bravo_wifi_power_state;
...
int __init bravo_init_mmc(unsigned int sys_rev, unsigned debug_uart)
{
	...
	if (system_rev > 0)
		...
		bravo_sdslot_data.status = bravo_sdslot_status_rev0;
		bravo_sdslot_data.register_status_notify = NULL;
		set_irq_wake(MSM_GPIO_TO_INT(BRAVO_GPIO_SDMC_CD_REV0_N), 1);
		msm_add_sdcc(2, &bravo_sdslot_data,
			     MSM_GPIO_TO_INT(BRAVO_GPIO_SDMC_CD_REV0_N),
...
#if defined(BRAVO_DEBUG_MMC) && defined(CONFIG_DEBUG_FS)
...
static int bravommc_dbg_sd_cd_set(void *data, u64 val)
{
	return -ENOSYS;
}
...
DEFINE_SIMPLE_ATTRIBUTE(bravommc_dbg_wifi_reset_fops,
			bravommc_dbg_wifi_reset_get,
			bravommc_dbg_wifi_reset_set, "%llu\n");

DEFINE_SIMPLE_ATTRIBUTE(bravommc_dbg_wifi_cd_fops,
			bravommc_dbg_wifi_cd_get,
			bravommc_dbg_wifi_cd_set, "%llu\n");

DEFINE_SIMPLE_ATTRIBUTE(bravommc_dbg_wifi_pwr_fops,
			bravommc_dbg_wifi_pwr_get,
			bravommc_dbg_wifi_pwr_set, "%llu\n");

DEFINE_SIMPLE_ATTRIBUTE(bravommc_dbg_sd_pwr_fops,
			bravommc_dbg_sd_pwr_get,
			bravommc_dbg_sd_pwr_set, "%llu\n");

DEFINE_SIMPLE_ATTRIBUTE(bravommc_dbg_sd_cd_fops,
			bravommc_dbg_sd_cd_get,
			bravommc_dbg_sd_cd_set, "%llu\n");
...
device_initcall(bravommc_dbg_init);
#endif
